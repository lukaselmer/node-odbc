  ...
  describe('...with promises...', () => {
    ...
    it('...should correctly identify function signature with .query({string}, {array}, {object})', async () => {
      ...
      const cursor = await pool.query(
        `SELECT * FROM ${process.env['DB_SCHEMA']}.${process.env['DB_TABLE']} WHERE ID = ?`,
        [1],
        { cursor: true },
      );
      ...
      describe('...[cursor]...', () => {
        it('...should throw an error if cursor is not a boolean or string', async () => {
          ...
          await assert.rejects(
            pool.query(`SELECT * FROM ${process.env['DB_SCHEMA']}.${process.env['DB_TABLE']}`, {
